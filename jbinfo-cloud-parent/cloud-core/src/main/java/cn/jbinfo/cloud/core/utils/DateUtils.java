 ...
 * <p>功能:日期类型的工具类，提供日期的转换操作，该类均为静态方法，直接调用
 */
public class DateUtils {
    ...
    public static Date parseDate(final String source, String pattern) {
        try {
            return parse(source, pattern);
        } catch (ParseException e) {
            LOG.error("DateUtils parse date is exception", e);
        }
        return null;
    }
     ...
     * @param source2 要比较的日期
     * @return 天数，之后大于零，之前小于零
     */
    public static int getDateToDays(final Date source1, final Date source2) {
        // 如果要比较的source不存在
    ...
    /**
     * 功能:获取两个日期之间天数，字符串参数方式
     * <p>作者文齐辉 2015年11月30日 上午10:39:16
     *
     * @param source1
     * @param source2
     * @return
     */
    public static int getDateToDays(final String source1, final String source2) {
        Date now = null, old = null;
        try {
            now = parse(source1, PATTERN_YYYY_MM_DD);
            old = parse(source2, PATTERN_YYYY_MM_DD);
        } catch (ParseException e) {
            throw new IllegalArgumentException(" getDateToDays format exception", e);
        }
        return getDateToDaysSecond(now, old);
    }
     ...
     * @return 天数，之后大于零，之前小于零
     */
    public static int getDateToDaysSecond(final Date source1, final Date source2) {
        // 如果要比较的source不存在
        if (source1 == null || source2 == null) {
            return 0;
        }
    ...
    public static int distance(Date before, Date after, int type) {
        ...
        long unit = -1;
        switch (type) {
            case Calendar.DAY_OF_MONTH:    //以天为单位
                end.set(Calendar.HOUR_OF_DAY, 0);
                start.set(Calendar.HOUR_OF_DAY, 0);
                unit = UNIT_DAY_TIME;
                break;
            case Calendar.HOUR_OF_DAY:    //以小时为单位
                unit = UNIT_HOUR_TIME;
                //设置分秒
                ...
                break;
            case Calendar.MINUTE:        //以分钟为单位
                unit = UNIT_MINUS_TIME;
                //设置分秒
                end.set(Calendar.SECOND, 0);
                start.set(Calendar.SECOND, 0);
                break;
            case Calendar.SECOND:        //以秒为单位
    ...
    public static Date getLastDayOfCurrentWeek(Date date) throws ParseException {
        ...
        date = parse(sToday, PATTERN_YYYY_MM_DD_HH_MM_SS);
        return date;
    }
    ...
    /**
     * 功能:当前服务器时间往后加几天
     * <p>作者文齐辉 2016年8月30日 下午5:13:28
     *
     * @param dayNum 需要加的天数
     * @return
     */
    public static String getServiceLimitDate(int dayNum) {
        fromCal.setTime(new Date());
        fromCal.add(Calendar.DATE, dayNum);
    ...
    public static Date addMinutes(Date date, int minutes) {
        return addByDate(date, Calendar.MINUTE, minutes);
    }
