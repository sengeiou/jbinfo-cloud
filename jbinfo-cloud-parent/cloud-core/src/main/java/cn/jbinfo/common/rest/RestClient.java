...
public class RestClient {
    ...
    public static <T> T exchange(String url, HttpMethod method, HttpHeaders httpHeaders, Object body,
        ...
        try {
            ...
            if (uriVariables.length == 1) {
            	if( uriVariables[0] instanceof Map){
            		Map<String, ?> _uriVariables = (Map<String, ?>) uriVariables[0];
            		return getClient().exchange(url, method, requestEntity, responseType, _uriVariables).getBody();
            	}
            }
    ...
    public static RestTemplate getClient() {
        return HttpPoolClientBuilder.build();
    }
    ...
    public static HttpHeaders buildJSONHeaders(Object... arrays) {
        ...
        for (int i = 0; i < arrays.length; i++) {
            headers.add(StringUtils.defaultString((String)arrays[i], ""), StringUtils.defaultString((String)arrays[++i], ""));
        }
        return headers;
    }
    ...
    public static void main(String[] args) {
        ...
        map.put("cardNum", "6222520914274432");
        CardNum cardNum = new CardNum();
        cardNum.setCardNum("6222520914274432");
        String url = "https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=6222520914274432&cardBinCheck=true";
        Map<String, Object> json = exchange(url, HttpMethod.POST, Map.class, cardNum);
        //{"bank":"COMM","validated":true,"cardType":"CC","stat":"ok","messages":[],"key":"1467088866936-3605-10.208.32.26-1709396386"}
