...
public class XmlNamedTemplateResolver implements NamedTemplateResolver {
	...
	@Override
	public Iterator<Void> doSqlInTemplateResource(Resource resource, final NamedTemplateCallback callback)
			throws Exception {
	...
	@Override
	public Iterator<Void> doHqlInTemplateResource(Resource resource, NamedTemplateCallback callback) throws Exception {
		InputSource inputSource = new InputSource(resource.getInputStream());
		inputSource.setEncoding(encoding);
		Document doc = documentLoader.loadDocument(inputSource, entityResolver, errorHandler,
				XmlValidationModeDetector.VALIDATION_XSD, false);
		final List<Element> sqes = DomUtils.getChildElementsByTagName(doc.getDocumentElement(), "hql");
